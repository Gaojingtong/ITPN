...
from selected_data.data_process import CrossDataset, SelectedDataset, SelectedDataset25, SelectedDatasetAmazom, MergeDataset, save_selected_data, clear_selected_data, backup_best_data, record_excel
...
def get_model(name, field_dims):
    ...
    if name == 'lr':
        ...
        return CLFM(field_dims, user_embedding_size=16, item_embedding_size=16, share_embedding_size=8, alpha=0.5,
                    reg_weight=1e-4)
    elif name == 'dtcdr':
        return DTCDR(field_dims, embedding_size=64, mlp_hidden_size=[128, 128], dropout_prob=0.3, alpha=0.3,
                     base_model="NeuMF")
    else:
...
def train_cdr_pre(model, optimizer, data_loader, device):
    model.train()
    # tk0 = tqdm.tqdm(data_loader, smoothing=0, mininterval=1.0)
...
def train_cdr(field_dims, train_data_loader, valid_data_loader, test_data_loader,
    ...
    for epoch_i in range(epoch):
        train_cdr_pre(model, optimizer, train_data_loader, device)
        auc, logloss = test_pre(model, valid_data_loader, device)
        print('\tepoch:', epoch_i,
              'validation: auc:', auc, 'logloss:', logloss)
        if not early_stopper.is_continuable(model, auc):
