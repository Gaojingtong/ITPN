...
class CLFM(torch.nn.Module):
    def __init__(self, field_dims, user_embedding_size, item_embedding_size, share_embedding_size, alpha, reg_weight):
        ...
        self.source_item_embedding = FeaturesEmbedding(field_dims[1:], item_embedding_size)
        self.target_item_embedding = FeaturesEmbedding(field_dims[1:], item_embedding_size)

        self.user_embedding_size = user_embedding_size
        self.source_item_embedding_size = item_embedding_size*(len(field_dims)-1)
        self.target_item_embedding_size = item_embedding_size*(len(field_dims)-1)
        self.share_embedding_size = share_embedding_size
        self.alpha = alpha
        self.reg_weight = reg_weight
        assert 0 <= share_embedding_size <= item_embedding_size*(len(field_dims)-1)
    ...
    def forward_emb(self, user_e, item_e, is_target=True):
        ...
        item_e = item_e.reshape((item_e.shape[0], -1))
        factors = []
        if is_target:
            if self.share_embedding_size > 0:
    ...
    def forward(self, x):
        user_embedding = self.target_user_embedding(x[:,0]).squeeze()
    ...
    def calculate_loss(self, x, label, cross):
        ...
        if target_idx is None or target_idx.dim == 0:
            ...
            target_user = self.target_user_embedding(torch.gather(
                user, 0, target_idx)).squeeze()
            target_item = self.target_item_embedding(torch.gather(
                item, 0, target_idx.reshape((-1, 1)).repeat(1, item.shape[1])))
        ...
        if source_idx is None or source_idx.dim == 0:
            ...
            source_user = self.source_user_embedding(torch.gather(
                user, 0, source_idx)).squeeze()
            source_item = self.source_item_embedding(torch.gather(
                item, 0, source_idx.reshape((-1, 1)).repeat(1, item.shape[1])))
