...
class DTCDR(torch.nn.Module):
    ...
    def calculate_loss(self, fields, label, cross, overlap_user):
        index = []
        noindex = []
        ...
        loss_s_o = True
        loss_s_no = True
        if fields_o is not None:
            user_o = fields_o[:, 0]
            item_o = fields_o[:, 1:]
            target_idx = torch.nonzero(cross_o).squeeze()
            ...
            source_idx = torch.nonzero(1-cross_o).squeeze()
        ...
        if fields_no is not None:
            user_no = fields_no[:, 0]
            item_no = fields_no[:, 1:]
            target_idx = torch.nonzero(cross_no).squeeze()
            if target_idx is None or target_idx.dim == 0:
                loss_t_no = False
            else:
            ...
            source_idx = torch.nonzero(1-cross_no).squeeze()
            if source_idx is None or source_idx.dim == 0:
                loss_s_no = False
            else:
